<template>
  <div class="page">
    <header class="top">
      <h1>MoQ Push</h1>
      <p class="sub">Camera + mic → H.264 / AAC → MOQT PUBLISH + LOC → WebTransport</p>
    </header>
...
import { defaultMoqPushUrl } from '../config'
...
const address = ref(defaultMoqPushUrl())
...
async function connect() {
  ...
  log_info('ui-moq-push', `click connect ${url}`)
  try {
    await pipeline.start(url, videoEl.value, {
      onStatus(s) {
        ...
        log_info('ui-moq-push', hint.value)
      },
      onError(msg) {
        hint.value = msg
      },
    ...
    log_error('ui-moq-push', `connect failed: ${msg}`)
...
async function disconnect() {
  stopping.value = true
  log_info('ui-moq-push', 'click disconnect')
  try {
    await pipeline.stop()
  } finally {
    connected.value = false
