<template>
  <div class="page">
    <header class="top">
      <h1>HTTP-FLV Pull</h1>
      <p class="sub">HTTP-FLV → flv.js playback</p>
    </header>
...
const puller = new HttpFlvPuller()
...
async function connect() {
  ...
  if (!url) {
    error.value = 'Enter an HTTP-FLV pull URL'
    return
  }
  ...
  log_info('ui-http-flv', `click connect ${url}`)
  try {
    await puller.start(url, videoEl.value, {
      onStats(s) {
        stats.value = s
      },
      onPhase(p) {
    ...
    log_error('ui-http-flv', `connect failed: ${msg}`)
...
async function disconnect() {
  stopping.value = true
  log_info('ui-http-flv', 'click disconnect')
  try {
    await puller.stop()
  } finally {
