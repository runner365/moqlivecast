<template>
  <div class="page">
    <header class="top">
      <h1>WT FLV Pull</h1>
      <p class="sub">WebTransport → FLV → flv.js playback</p>
    </header>
    ...
    <section class="bar">
      ...
      <input
        ...
        class="input"
        :disabled="busy"
        placeholder="https://host:4433/flv?method=pull&amp;app=live&amp;stream=<stream>"
      />
...
import { defaultPullUrl } from '../config'
...
import { WtPuller } from '../media/wtPuller'

const address = ref(defaultPullUrl('flv'))
...
async function connect() {
  ...
  log_info('ui-pull', `click connect ${url}`)
  try {
    ...
    log_error('ui-pull', `connect failed: ${msg}`)
...
async function disconnect() {
  stopping.value = true
  log_info('ui-pull', 'click disconnect')
  try {
    await puller.stop()
  } finally {
